...
######################################################################
##### The input is always of shape (N,1)##############
def cut_piece(ts, perc=.8,typeC = 3):
    seq_len = ts.shape[0]
    win_class = seq_len / (2 * typeC)    
#     win_class = 2*seq_len / (3 * typeC)
    ...
    for i in range(typeC):
        if abs(start1 - start2) >= typeC* win_class:
            label = typeC-1
        if abs(start1 - start2) >= i* win_class  and abs(start1 - start2) < (i+1)* win_class :
            label = i

    return ts[start1:end1, ...], ts[start2:end2, ...], label

def cut_one_piece(ts, piece_len= 800 ):
    seq_len = ts.shape[0]
    win_len = piece_len
    ...
    return ts[start1:end1, ...]



# def cut_piece(ts, perc=.1,typeC = 3):
...
#     new_ts1 = np.zeros_like(ts)            
#     new_ts2 = np.zeros_like(ts)
#     new_ts1[start1:end1, ...] = ts[start1:end1, ...]
#     new_ts2[start1:end1, ...] = ts[start1:end1, ...]
#     return new_ts1, new_ts2, label
...
############################################=my=###########################################
##### The input is always of shape (N,1)##############
def time_warp_yue(x, sigma=0.2, knot=4):
    from scipy.interpolate import CubicSpline
    ...
    for i, pat in enumerate(x):
        ...
        orig_steps = np.arange(pat.shape[0])
        random_warps = np.random.normal(loc=1.0, scale=sigma, size=(knot + 2, pat.shape[1]))
        warp_steps = (np.ones((pat.shape[1], 1)) * (np.linspace(0, pat.shape[0] - 1., num=knot + 2))).T
        
        tt_cum = np.zeros_like(pat)
        tt_new = np.zeros_like(pat)
        t_scale = np.zeros(pat.shape[1])
        for dim in range(pat.shape[1]):
            
            time_warp = CubicSpline(warp_steps[:, dim],
                                    random_warps[:, dim])(orig_steps)
#             print('time_warp:',time_warp.shape,time_warp)

            tt_cum[:,dim] = np.cumsum(time_warp, axis=0)       # Add intervals to make a cumulative graph
            t_scale[dim] = (pat.shape[0]-1)/tt_cum[-1,dim]

            tt_new[:,dim] = tt_cum[:, dim]*t_scale[dim]
#             print('tt_cum:',tt_cum.shape, tt_cum[:,dim])
            ret[i,:, dim] = np.interp(orig_steps, tt_new[:,dim],  pat[:, dim]).T
#         print('t_scale:',t_scale)
#         print('tt_new:',tt_new[-10:-1,:])
    return ret


##### The input is always of shape (N,1)##############
def time_warp_s_yue(x, sigma=0.2, knot=4, plot=False):
    from scipy.interpolate import CubicSpline
...
#         print('time_warp:',time_warp.shape,time_warp)
        ...
        tt_cum[:,dim] = np.cumsum(time_warp, axis=0)       # Add intervals to make a cumulative graph
        t_scale[dim] = (x.shape[0]-1)/tt_cum[-1,dim]
        
        tt_new[:,dim] = tt_cum[:, dim]*t_scale[dim]
#         print('tt_cum:',tt_cum.shape, tt_cum[:,dim])
        ret[:, dim] = np.interp(orig_steps, tt_new[:,dim],  x[:, dim]).T
#     print('t_scale:',t_scale)
#     print('tt_new:',tt_new[-10:-1,:])
