# -*- coding: utf-8 -*-
### modified based on https://github.com/haoyfan/SelfTime/blob/master/dataloader/ucr2018.py ###
...
class UCR2018_Peaks(data.Dataset):
    ...
    def __init__(self, data, targets, targets_peak, transform):
        self.data = np.asarray(data, dtype=np.float32)
        self.targets = np.asarray(targets, dtype=np.int64)
#         self.targets_peak = np.asarray(targets_peak, dtype=np.float32)
        self.targets_peak = np.asarray(targets_peak, dtype=np.int64)
        self.transform = transform

    def __getitem__(self, index):
        img, target, targets_peak = self.data[index], self.targets[index], self.targets_peak[index]
        if self.transform is not None:
            img_transformed = self.transform(img.copy())
        else:
            img_transformed = img

        return img_transformed, target, targets_peak
...
# #         print('img_list:',len(img_list),img_list[0].shape,self.data.shape[0])
#         ####################################################################
...
class MultiUCR2018_InterIntra(data.Dataset):
    ...
    def __init__(self, data, targets, K, transform, transform_cut, totensor_transform):
        self.data = np.asarray(data, dtype=np.float32)
        #############################################################
        self.targets = targets #np.asarray(targets, dtype=np.int16)
        #############################################################
        self.K = K  # tot number of augmentations
        self.transform = transform
        self.transform_cut = transform_cut
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            for _ in range(self.K):
#                 print('MultiUCR2018_InterIntra img shape:',type(img),len(img),img.shape )
                img_transformed = self.transform(img.copy())
                img_cut0, img_cut1, label = self.transform_cut(img_transformed)
                img_list.append(self.totensor_transform(img_transformed))
                img_list0.append(self.totensor_transform(img_cut0))
...
#         print('img_list:',len(img_list),img_list[0].shape,self.data.shape[0])
        ...
        return img_list, img_list0, img_list1, label_list, target_list
...
class MultiUCR2018_InterIntra_Moco(data.Dataset):

    def __init__(self, data, targets, K, transform, transform_cut, totensor_transform, moco_transform):
        self.data = np.asarray(data, dtype=np.float32)
        #############################################################
        self.targets = targets #np.asarray(targets, dtype=np.int16)
        #############################################################
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img_moco = self.moco_transform(img.copy())
...
#         print('img_list:',len(img_list),img_list[0].shape,self.data.shape[0])
        ...
        return img_moco, img_list, img_list0, img_list1, label_list, target_list
